/**
 * plugin-bailongma-memory —— 白龙马记忆系统迁移到 DeepSeek Harness。
 *
 * 核心能力：
...
export const name = 'plugin-bailongma-memory'

export const inject = ['sessions', 'tools', 'llm']

export interface MemoryPluginConfig {
  /** 记忆库数据目录（默认 $DSH_HOME/bailongma-memory） */
  root?: string
  /** 记忆注入条数 */
  injectLimit?: number
...
export interface BailongmaMemoryService {
  store: MemoryStore
  focus: FocusStack
...
export function apply(ctx: Context, config: MemoryPluginConfig = {}) {
  ...
  ctx.logger.info(`[bailongma-memory] 记忆库已打开: ${store.dbPath}（${store.count()} 条记忆）`)
    ...
    if (event.type === 'user/message') {
      ...
      if (llmRecognition && turnUserText.trim().length >= 4 && !recognitionRunning) {
        ...
        void runRecognition(ctx, store, userText, assistantText, recognizerConfig, recognitionAbort.signal)
          .then(written => {
            if (written > 0) ctx.logger.info(`[bailongma-memory] LLM 识别器写入 ${written} 条长期记忆`)
          })
    ...
    if (type === 'user/message') {
      ...
      if (text) {
        ...
        if (result.framesToCompress.length > 0) {
          ...
          ctx.logger.info(`[bailongma-memory] 焦点栈失活/回归，压缩回填 ${result.framesToCompress.length} 帧`)
        }
        // 启发式偏好提取
        if (config.heuristicProfile !== false) {
          ...
          if (pref) {
            ...
            ctx.logger.info(`[bailongma-memory] 学习到用户偏好: ${pref.value}`)
          }
        }
        // 线程分类（话题漂移 + 指代恢复）
        const threadCls = threads.onUserMessage(text)
        if (threadCls.kind !== 'none') {
          ctx.logger.info(`[bailongma-memory] 线索: ${threadCls.kind}${threadCls.kind !== 'new' ? ` #${threadCls.threadId}` : ''}（开放 ${threads.threads.length} 条）`)
        }
      }
  ...
  ctx.logger.info(`[bailongma-memory] 已注册 ${tools.length + 1} 个记忆工具`)

  /* ============ 服务暴露 ============ */

  const service: BailongmaMemoryService = {
    store,
    focus,
    threads,
  ...
  ctx.provide('bailongma.memory', service)
  ...
  ctx.effect(() => {
    const decayTimer = setInterval(() => {
      ...
      if (changed > 0) ctx.logger.info(`[bailongma-memory] 记忆衰减完成，处理 ${changed} 条`)
    }, decayInterval)
    return () => {
