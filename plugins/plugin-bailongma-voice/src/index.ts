/**
 * plugin-bailongma-voice —— 白龙马语音栈迁移（TTS 部分）。
 *
 * 能力：
...
export const name = 'plugin-bailongma-voice'
export const inject = ['tools']

export interface VoiceConfig {
  port?: number
  provider?: string
  /** 数据目录（默认 $DSH_HOME/bailongma-voice） */
  root?: string
...
export function apply(ctx: Context, config: VoiceConfig = {}) {
  ...
  const root = config.root ?? `${process.env.DSH_HOME ?? `${process.env.USERPROFILE ?? '~'}/.dsh`}/bailongma-voice`
  ...
  async function synthesize(p: string, text: string): Promise<Buffer> {
    ...
    if (p === 'doubao') {
      ...
      const res = await fetch('https://openspeech.bytedance.com/api/v3/tts', {
        method: 'POST',
        headers: { 'content-type': 'application/json', authorization: `Bearer; ${apiKeyOf(p)}` },
        body: JSON.stringify({ app: { appid: apiKeyOf(p) }, user: { uid: 'marvis' }, audio: { voice_type: voice, encoding: 'mp3', speed_ratio: 1.0 }, request: { reqid: randomUUID(), text, operation: 'query' } }),
      })
  ...
  const server = createServer((req, res) => {
    ...
    if (url === '/') {
      ...
      res.end('Bailongma Voice Service\nGET /api/tts-config\nGET /audio/:id\n')
      return
    }
  ...
  ctx.effect(() => {
    server.listen(port, '127.0.0.1', () => {
      ctx.logger.info(`[bailongma-voice] 🔊 语音服务: http://127.0.0.1:${port}/`)
    })
    return () => server.close()
  })
  ...
  ctx.logger.info(`[bailongma-voice] 已注册 speak 工具（provider=${provider || '未配置'}）`)

  /* ============ 服务暴露 ============ */

  ctx.provide('bailongma.voice', {
