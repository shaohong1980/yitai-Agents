/**
 * MarvisOffice —— 办公室团队引擎门面。
 *
 * 借鉴 dsh-agent-teams 的架构：磁盘真相源 + 事件调度 + durable 成员 + attempt 能力，
 * 同时桥接现有办公室可视化（MarvisTeam / WS 广播），保持 UI 契约不变。
 *
 * 设计：
...
import type { MarvisMember, MarvisMessage, MarvisOfficeState, MarvisTask, MarvisTaskStatus } from './types.ts'
import { installOfficeScheduler, type TeamScheduler } from './scheduler.ts'
import { AGENTS, type AgentDef, type TeamEvent, type MarvisTeam } from './team.ts'
...
/** 固定工位员工定义（file/computer/app/zhuge/find，不含雷总管）。 */
export const WORKER_DEFS: AgentDef[] = AGENTS.filter((a) => a.id !== 'marvis')

/** 办公室 UI 快照（复用 MarvisTeam.snapshot 形状 + durable 任务）。 */
export interface OfficeSnapshot {
  agents: ReturnType<MarvisTeam['snapshot']>['agents']
  doneCount: number
  logs: ReturnType<MarvisTeam['snapshot']>['logs']
  tasks: MarvisTask[]
  office: MarvisOfficeState | undefined
}

/** 办公室配置。 */
export interface MarvisOfficeConfig {
...
export interface MarvisOfficeApi {
  ...
  /** 广播任务（雷总管拆解 → 建任务 → 调度/模拟）。 */
  dispatch(text: string, captain?: Agent): Promise<{ task: MarvisTask; delegated: boolean }>
  /** 认领任务，返回 attemptId。 */
  claimTask(taskId: string, memberName: string): Promise<string>
  /** 携带 attemptId 推进任务状态。 */
  updateTask(taskId: string, attemptId: string, status: MarvisTaskStatus, output?: string): Promise<MarvisTask>
  /** 转派任务（安全接管）。 */
  reassignTask(taskId: string, toMember: string): Promise<MarvisTask>
  /** 原地补丁任务字段（如 comment 驳回意见）。 */
  patchTask(taskId: string, patch: Partial<Pick<MarvisTask, 'comment'>>): Promise<MarvisTask>
  /** 成员/队长互发消息（durable 邮箱 + 唤醒）。 */
  sendMessage(from: string, to: string, content: string): Promise<void>
  /** 读取某人的未读邮箱。 */
  readMailbox(key: string): Promise<MarvisMessage[]>
  ...
  /** 磁盘真相：办公室状态。 */
  state(): Promise<MarvisOfficeState | undefined>
  /** UI 快照（复用 MarvisTeam.snapshot + 任务/邮箱补充）。 */
  snapshot(): Promise<OfficeSnapshot>
}

/** 创建一个办公室引擎实例。 */
export function createMarvisOffice(
  ctx: Context,
  team: MarvisTeam,
  broadcast: (event: TeamEvent) => void,
  config: MarvisOfficeConfig,
): MarvisOfficeApi {
  ...
  async function loadOffice(): Promise<MarvisOfficeState | undefined> {
    await readyPromise
    return readOffice(stateRoot)
  }

  /** 种子：固定工位员工。 */
  function seedMembers(): MarvisMember[] {
  ...
  async function init(captainSessionId?: string): Promise<void> {
    ...
    try {
      await ensureOffice(stateRoot, {
        name: 'Marvis 办公室',
        id: officeId,
        description: '多Agent办公室：雷总管 + 5 名专职员工（durable 团队）',
  ...
  async function claimTask(taskId: string, memberName: string): Promise<string> {
    return withOfficeLock(lockKey, async () => {
      ...
      if (member === undefined && memberName !== 'marvis' && memberName !== 'captain') {
  ...
  async function reassignTask(taskId: string, toMember: string): Promise<MarvisTask> {
    return withOfficeLock(lockKey, async () => {
      const office = await loadOffice()
      if (office === undefined) throw new Error('办公室尚未初始化')
      const task = office.tasks.find((t) => t.id === taskId)
      ...
      if (member === undefined && toMember !== 'marvis' && toMember !== 'captain') {
  ...
  async function patchTask(
    taskId: string,
    patch: Partial<Pick<MarvisTask, 'comment'>>,
  ): Promise<MarvisTask> {
    return withOfficeLock(lockKey, async () => {
      const office = await loadOffice()
      if (office === undefined) throw new Error('办公室尚未初始化')
  ...
  async function ensureMembers(captain: Agent): Promise<void> {
    ...
    for (const member of office.members) {
      ...
      try {
        ...
        await writeOffice(stateRoot, office)
        ctx.logger.info(`[marvis-office] 🤖 员工 ${member.name} 已 spawn 为 durable subagent：${member.id}`)
      } catch (error: unknown) {
        ctx.logger.warn(`[marvis-office] 员工 ${member.name} spawn 失败（回退模拟）：${String(error)}`)
        break // 一个失败往往意味着 provider/凭据整体不可用
      }
    }
  }

  async function dispatch(text: string, captain?: Agent): Promise<{ task: MarvisTask; delegated: boolean }> {
    ...
    if (config.liveDelegation && captain) {
      ...
      ctx.logger.warn('[marvis-office] 无可用真实 subagent 成员，回退可视化模拟')
  ...
  async function state(): Promise<MarvisOfficeState | undefined> {
    return loadOffice()
  }
