 ...
 * 队长用 `ctx.subagents.followup` 唤醒它，它工作完整一轮（通过 `yitai_update_task`
 * 等工具推进任务状态），随后回到 idle。最终 assistant 消息不可程序化读取，所以
 * 员工把结果写进任务记录与队长邮箱，队长用 `yitai_status` 读取。
 */
import type { Context } from '@deepseek-ai/cordis'
import type { Agent } from '@deepseek-ai/dsh-agent'
import type { SessionId } from '@deepseek-ai/dsh-session'
import type { YitaiMember, YitaiOfficeState } from './types.ts'
...
const MEMBER_LABEL_PREFIX = 'yitai-member:'
...
export function memberPersona(office: YitaiOfficeState, member: YitaiMember, stateDir: string): string {
  return `你是 ${member.name}（${member.role}），多Agent办公室「${office.name}」的一名专职员工。队长（易总管）领导办公室；你是一名执行员工。

团队上下文：
- 办公室 id：${office.id}
- 你的身份 name：${member.name}
- 办公室状态位于 ${stateDir}/（office.json 和 inbox/*.jsonl）。只可只读诊断，绝不直接编辑；用 yitai_* 工具做更新，保证 JSON 转义与并发安全。
- 队长通过消息给你派活。每收到一条消息就是新的一轮：执行它并以简短回复收尾。

工作规则：
1. 收到任务派发时，用 yitai_claim_task 领取（传入任务 id）。记下返回的 attempt_id：之后的 yitai_update_task 都要带上它。
2. 认真用你手头的工具（bash/fs/web 等）完成工作，不要偷工减料。
3. 完成后用 yitai_update_task（带同一个 attempt_id，status=completed，output 简明总结做了什么与关键结果）。stale-attempt 拒绝 = 队长已转派/接管，停止该任务，等待新工作。
4. 完成后给队长发一句简短报告：yitai_send_message（to=captain）。
5. 需要向同事提问时用 yitai_send_message（to=<员工name>），消息直达对方邮箱并唤醒。
6. 你是执行员工：不要创建/删除办公室、不要转派任务、不要派发新任务——那是队长的职责。`
}

/** 员工创建时的欢迎语。 */
export function memberWelcome(office: YitaiOfficeState): string {
...
export async function spawnMember(
  ...
  if (provider === undefined) {
    throw new Error(
      `yitai: 没有注册 subagent provider "spawn"（可用：${ctx.subagents.list().join(', ') || 'none'}）`
      + ' — 请确认 subagent-spawn 已挂载进组合',
    )
  }
  if (provider.prepareContinuable === undefined) {
    throw new Error(`yitai: provider "spawn" 不支持 durable 可续聊成员（缺 prepareContinuable）`)
  }
  if (!provider.capabilities.persona) {
    throw new Error(`yitai: provider "spawn" 无法为成员应用 persona`)
  }
...
export async function deliverToMember(
  ...
  try {
    await ctx.subagents.followup(captain, brandedSessionId(childId), [{ type: 'text', text }], {
      source: { kind: 'plugin', plugin: 'yitai-orchestrator' },
      signal,
    })
    return true
  } catch (error: unknown) {
    ctx.logger.warn(`yitai: 对成员 ${childId} 的 followup 失败：${String(error)}`)
    return false
  }
}
...
export function interruptMember(ctx: Context, captain: Agent, childId: string): void {
  try {
    ...
    ctx.logger.warn(`yitai: 中断成员 ${childId} 失败：${String(error)}`)
  }
}
