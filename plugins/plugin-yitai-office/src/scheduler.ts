...
import type { YitaiMessage, YitaiTask } from './types.ts'
...
function ownedOpenTask(tasks: readonly YitaiTask[], memberName: string): YitaiTask | undefined {
  return tasks.find((task) => task.assignee === memberName
    && (task.status === 'claimed' || task.status === 'in_progress'))
}

/** 下一条 ready 任务：优先该成员名下的，其次未指派共享池。 */
function nextReadyTask(tasks: readonly YitaiTask[], memberName: string): YitaiTask | undefined {
...
function assignmentPrompt(ticket: DispatchTicket, stateDir: string, officeId: string): string {
  ...
  return `Yitai 办公室自动任务派发（共享任务池）。

任务：${ticket.taskId} — ${ticket.subject}${description}
Attempt：${ticket.attempt}
Attempt id：${ticket.attemptId}

请用 yitai_claim_task 认领任务 ${ticket.taskId}，它会返回同一个 attempt_id；之后每次 yitai_update_task 都带上 attempt_id=${ticket.attemptId}。若被拒绝为 stale，说明任务已被转派，停止工作。本轮只做这一个任务，完成后把结果汇报给队长，然后回到 idle 等待调度器给你下一份 ready 任务。

状态目录：${stateDir}/${officeId}/ 只读诊断；只用 yitai_* 工具改办公室状态。`
}

/** fallback 邮箱投递提示词。 */
function fallbackMailboxPrompt(messages: readonly YitaiMessage[]): string {
  return [
    'Yitai 办公室投递了实时投递不可用期间持久化的消息：',
    ...messages.map((message) => `\n来自 ${message.from}：\n${message.content}`),
    '\n本轮处理这些消息。任务派发仍需要 yitai_claim_task + 当前 attempt_id。',
  ].join('\n')
}
...
export function installOfficeScheduler(ctx: Context, config: OfficeSchedulerConfig): TeamScheduler {
  ...
  ctx.on('agent/status', ({ agent, status }) => {
    void syncMemberStatus(agent, status).catch((error: unknown) => {
      ctx.logger.warn(`yitai: 成员状态调度失败（${agent.id}）：${String(error)}`)
    })
  })
