/**
 * Yitai 办公室状态持久化与纯逻辑规则 —— 借鉴 dsh-agent-teams 的
...
import type { YitaiMessage, YitaiOfficeState, YitaiTask, YitaiTaskStatus } from './types.ts'
...
/** 默认状态目录名。 */
export const DEFAULT_STATE_DIR = '.yitai-office'
...
/** 同步读办公室状态（冷恢复路径用）。 */
export function readOfficeSync(stateRoot: string): YitaiOfficeState | undefined {
  try {
    const raw = readFileSync(officeFileOf(stateRoot), 'utf8')
    return JSON.parse(raw) as YitaiOfficeState
  } catch {
    return undefined
  }
}
...
/** 异步读办公室状态。 */
export async function readOffice(stateRoot: string): Promise<YitaiOfficeState | undefined> {
  try {
...
/** 原子写办公室状态（tmp + rename）。 */
export async function writeOffice(stateRoot: string, state: YitaiOfficeState): Promise<void> {
...
export async function ensureOffice(
  stateRoot: string,
  seed: Omit<YitaiOfficeState, 'createdAt'>,
): Promise<YitaiOfficeState> {
  const existing = await readOffice(stateRoot)
  if (existing) return existing
  const state: YitaiOfficeState = { ...seed, createdAt: Date.now() }
  await writeOffice(stateRoot, state)
  return state
}

/** 返回尚未满足的依赖任务 id（全满足为空数组）。 */
export function unsatisfiedDependencies(tasks: YitaiTask[], dependencies: string[]): string[] {
  const byId = new Map(tasks.map((task) => [task.id, task]))
  return dependencies.filter((id) => byId.get(id)?.status !== 'completed')
}

/** 校验任务状态迁移。 */
export function transitionError(current: YitaiTaskStatus, next: YitaiTaskStatus): string | undefined {
...
export function beginTaskAttempt(task: YitaiTask, assignee: string): string {
...
export function invalidateTaskAttempt(task: YitaiTask): void {
...
/** 追加一条消息到某邮箱。 */
export async function appendMailbox(stateRoot: string, message: YitaiMessage): Promise<void> {
...
export async function readUnreadMailbox(stateRoot: string, key: string): Promise<YitaiMessage[]> {
  ...
  const now = Date.now()
  const out: YitaiMessage[] = []
  for (const line of lines) {
    let msg: YitaiMessage
...
async function mutateMailbox(
  stateRoot: string,
  key: string,
  fn: (msg: YitaiMessage) => YitaiMessage,
): Promise<void> {
  const file = mailboxFileOf(stateRoot, key)
  let lines: string[] = []
  try {
