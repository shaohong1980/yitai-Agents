/**
 * Durable Yitai office state types —— 借鉴 dsh-agent-teams 的磁盘真相源模型。
 *
 * 办公室状态持久化在 `<workspace>/.yitai-office/office.json`：
...
/** 终态集合：终态后不可再被认领/执行。 */
export const TERMINAL_TASK_STATUSES: readonly YitaiTaskStatus[] = ['completed', 'failed', 'cancelled']

/** 任务状态机允许的迁移（review 可被驳回回 pending，对应审批流）。 */
export const TASK_TRANSITIONS: Readonly<Record<YitaiTaskStatus, readonly YitaiTaskStatus[]>> = {
...
export interface YitaiTask {
  ...
  /** 执行者：员工 id（file/computer/app/zhuge/find）或 yitai/captain；未指派等待认领。 */
  assignee?: string
  /** 必须先行完成的依赖任务 id 列表。 */
  dependencies: string[]
  /** 完成/失败时执行者写入的结果。 */
...
export type YitaiMemberStatus = 'idle' | 'working' | 'thinking' | 'reporting' | 'sleep' | 'removed'

/** 一名办公室员工：durable 可续聊 subagent + 团队侧记录。 */
export interface YitaiMember {
...
/** 完整 durable 办公室记录。 */
export interface YitaiOfficeState {
  name: string
  /** 净化后的目录 id。 */
  id: string
  description?: string
