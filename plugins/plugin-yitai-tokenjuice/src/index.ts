/**
 * plugin-yitai-tokenjuice —— 工具结果压缩（TokenJuice）。
 *
 * 基于 src/runtime/tool-result-compressor.js。
 *
...
export const name = 'plugin-yitai-tokenjuice'
...
export interface TokenJuiceConfig {
  ...
  /** 只压缩白名单内的只读/信息型工具 */
  whitelist?: string[]
  /** 输出目录（默认 $DSH_HOME/yitai-tokenjuice/outputs） */
  outputRoot?: string
  /** 单文件保存上限字符数 */
  maxSaveChars?: number
  /** 输出文件保留时长（天） */
...
export function apply(ctx: Context, config: TokenJuiceConfig = {}) {
  ...
  const outputRoot = config.outputRoot ?? `${process.env.DSH_HOME ?? `${process.env.USERPROFILE ?? '~'}/.dsh`}/yitai-tokenjuice/outputs`
